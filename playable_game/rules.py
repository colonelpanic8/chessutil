...
class ChessBoardWithRules(object):
    ...
    def is_square_threatened(self, square, by_color=common.color.WHITE):
        for i in range(8):
            for j in range(8):
    ...
    def make_legal_move(self, move_info):
        ...
        if piece.lower() == 'p' and move_info.destination[0] in (0, 7):
            if move_info.promotion not in ('Q', 'R', 'B', 'N'):
                ...
                if self.action == common.color.WHITE:
                    piece = piece.lower()
    ...
    def can_castle_kingside(self, color):
        return self._white_can_castle_kingside if color == common.color.WHITE \
            else self._black_can_castle_kingside

    def can_castle_queenside(self, color):
        return self._white_can_castle_queenside if color == common.color.WHITE \
    ...
    def _filter_moves_for_king_safety(self, start_position, moves):
        ...
        if piece.lower() == 'k':
            back_rank_index = 0 if piece_color is common.color.WHITE else 7
    ...
    def _get_castling_moves_for_color(self, color):
        back_rank = 0 if color is common.color.WHITE else 7
    ...
    def _check_pawn_capture_move(self, rank_index, file_index, color=common.color.WHITE):
    ...
    def _threatened_moves_for_pawn(self, rank_index, file_index):
        ...
        double_move_rank = 3 if piece_color is common.color.WHITE else 4
